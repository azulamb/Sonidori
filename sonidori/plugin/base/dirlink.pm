...
sub Inline()
{
  ...
  my $spath = $path;
  $spath =~ s/$sonidori->{ 'SYSTEM' }{ 'PUBLIC_DIR' }//;
  $spath =~ s/^\///;
  $spath =~ s/^\.//;
  $spath =~ s/\/+/\//g;
  ...
  if ( $spath ne '' )
  {
    ...
    my $updir = join( '/', @name );
    if ( $updir eq '' ){ $updir = '.'; }
    $html .= sprintf( '<li id="dir"><a href="%s?page=%s&%s">%s</a></li>%s', $sonidori->{ 'SYSTEM' }{ 'CGI' }, &Common::URLEncode( $updir ), $sonidori->CreateGetData(), '../', "\n" );
    $spath .= '/';
  }

  foreach ( @dirs )
  {
    $html .= sprintf( '<li id="dir"><a href="%s?page=%s&%s">%s/</a></li>%s', $sonidori->{ 'SYSTEM' }{ 'CGI' }, &Common::URLEncode( $spath . $_ ), $sonidori->CreateGetData(), $_, "\n" );
  }
  foreach ( @files )
  {
    my ( @name ) = split( /\./, $_ );
